...
By default, `install` and `remove` write to user settings (`~/.pp/agent/settings.json`). Use `-l` to write to project settings (`.pp/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup.
...
- User installs go under `~/.pp/agent/npm/`.
- Project installs go under `.pp/npm/`.
...
- Cloned to `~/.pp/agent/git/<host>/<path>` (global) or `.pp/git/<host>/<path>` (project).
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.pp/agent`) and project (`.pp/`) scopes.
