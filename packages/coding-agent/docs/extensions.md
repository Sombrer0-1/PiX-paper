...
> **Placement for /reload:** Put extensions in `~/.pp/agent/extensions/` (global) or `.pp/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.pp/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.pp/agent/extensions/*.ts` | Global (all projects) |
| `~/.pp/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.pp/extensions/*.ts` | Project-local |
| `.pp/extensions/*/index.ts` | Project-local (subdirectory) |
