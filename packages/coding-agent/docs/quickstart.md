...
Uninstalling pi leaves settings, credentials, sessions, and installed pi packages in `~/.pp/agent/`.
...
You can also run `/login` and select an API-key provider to store the key in `~/.pp/agent/auth.json`.
