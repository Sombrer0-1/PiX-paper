...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.pp/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.pp/extensions/`)
- Project skills:
  - `.pp/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.pp/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.pp/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.pp/agent/auth.json and ~/.pp/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.pp/agent/extensions/`, `.pp/extensions/`, and settings.json extension sources.
...
1. Global: `~/.pp/agent/settings.json`
2. Project: `<cwd>/.pp/settings.json`
