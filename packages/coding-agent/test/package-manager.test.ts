...
Content`,
		...
		it("should auto-discover root markdown skills from .pp skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .pp", async () => {
			const extDir = join(tempDir, ".pp", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".pp"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".pp", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".pp", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".pp", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".pp", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".pp", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .pp/agent skills", async () => {
		...
		it("should use the project .pp dir as baseDir for project .pp skills", async () => {
			const projectBaseDir = join(tempDir, ".pp");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".pp", "agent");
		...
		it("should dedupe user skill entries when ~/.pp/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .pp auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".pp\n");

			const skillDir = join(tempDir, ".pp", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.pp\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".pp", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .pp settings base", () => {
			...
			const rel = relative(join(tempDir, ".pp"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".pp", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".pp", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".pp", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".pp", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".pp", "npm", "node_modules", "example");
