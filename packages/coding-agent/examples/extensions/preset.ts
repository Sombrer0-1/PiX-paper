 ...
 * - ~/.pp/agent/presets.json (global)
 * - <cwd>/.pp/presets.json (project-local)
 *
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".pp", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.pp/agent/presets.json or .pp/presets.json", "warning");
			return;
		}
