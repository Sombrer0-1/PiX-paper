...
function api(): PixApi {
  if (!window.pixApi) {
    ...
    return {
      ...
      paperCreateProject: async () => ({ success: false }),
      paperValidateProjectDir: async () => ({ success: false }),
      paperCheckProject: async () => true,
      paperGetState: async () => ({ success: false, error: "unavailable" }),
      paperStartStage: async () => ({ success: false }),
      paperPauseStage: async () => ({ success: false }),
      paperResumeStage: async () => ({ success: false }),
      paperRespondGate: async () => ({ success: false }),
      paperListArtifacts: async () => ({ success: false, error: "unavailable" }),
      paperReadArtifact: async () => ({ success: false, error: "unavailable" }),
      paperMarkInboxItem: async () => ({ success: false }),
      paperOpenArtifact: async () => ({ success: false }),
      paperOpenInFolder: async () => ({ success: false }),
      onPaperGate: () => () => {},
      onPaperStateChanged: () => () => {},
      onPaperInboxChanged: () => () => {},
