  ...
  <div class="left-panel">
    ...
    <div class="panel-header">
      <div class="project-name-row">
        ...
        <span v-if="paperStore.isPaperMode" class="paper-badge" title="Paper 写作模式">Paper</span>
      </div>
      ...
      >
        ...
        <span class="hover-actions">
          <button
            ...
            :aria-label="pinnedIds.has(session.id) ? '取消置顶会话' : '置顶会话'"
          >
    ...
    <div class="panel-footer">
      <button type="button" class="footer-btn" @click="goSettings" title="设置" aria-label="打开设置">
      ...
      <button type="button" class="footer-btn" @click="goHome" title="主页" aria-label="返回主页">
    ...
    <v-dialog v-model="showNewSessionDialog" max-width="430">
      <v-card class="delete-dialog-card">
        <div class="delete-dialog-title">创建独立会话</div>
        <div class="delete-dialog-text">
          新会话不会加入当前 Paper 阶段流程。项目阶段与已有产物会保留，但该会话中的工作不会自动推进论文阶段。
        </div>
        <v-card-actions class="delete-dialog-actions">
          <v-spacer />
          <v-btn variant="text" @click="showNewSessionDialog = false">取消</v-btn>
          <v-btn color="primary" variant="flat" @click="createNewSession">继续创建</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
.search-input {
  ...
  color: var(--pix-text-primary);
  box-shadow: inset 0 1px 0 var(--pix-bg-content);
  transition:
