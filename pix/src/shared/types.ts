...
/** Snapshot returned by paper-get-state and pushed via paper-state-changed. */
...
/** Payload of the `paper-state-changed` event forwarded to the renderer. */
export interface PaperStateChangedEvent extends PaperStateSnapshot {}

export interface CreatePaperProjectPayload {
  projectDir: string;
  name: string;
  topic: string;
}

export interface StartPaperStagePayload {
  stage: StageId;
}

export interface PausePaperStagePayload {
  stage: StageId;
  reason?: string;
}

export interface MarkPaperInboxItemPayload {
  itemId: string;
  status: "open" | "read" | "resolved";
}

export interface RespondGatePayload {
  decision: GateDecision["decision"];
  reworkTarget?: StageId;
  reason?: string;
}

export interface ListArtifactsPayload {
  stage?: StageId;
}
...
export interface ValidatePaperProjectDirPayload {
  projectDir: string;
}
...
export interface PaperArtifactContent {
  ...
  path: string;
  mimeType: string;
  kind: "text" | "json" | "image" | "pdf" | "unsupported";
  content?: string;
  dataUrl?: string;
  byteSize?: number;
  truncated?: boolean;
