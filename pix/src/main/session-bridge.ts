...
import { ensureProjectConfig, type ProjectConfig } from "./project-init.js";
...
export class SessionBridge {
	...
	async start(projectDir: string, guiSettings?: GuiSettings): Promise<void> {
		...
		// Load or create .pp project config
		try {
			this._projectConfig = ensureProjectConfig(projectDir);
		} catch (err) {
			console.warn("[SessionBridge] Failed to load .pp project config:", err);
	...
	getState(): RpcSessionState {
		...
		return {
			...
			goal: session.goal,
			projectConfig: this._projectConfig ?? undefined,
		};
	}
